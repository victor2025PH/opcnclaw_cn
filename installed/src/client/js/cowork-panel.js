...
const STATE_META = {
  ready:    { icon: '\u{1F7E2}', label: 'AI 就绪',  color: 'var(--success)' },
...
function renderBadge() {
  ...
  _badge.classList.toggle('cw-working', st === 'working');
}
...
function renderPanel() {
  ...
  const canOperate = _status.can_operate_desktop;
  const running = _status.tasks_running || 0;
  const pending = _status.tasks_pending || 0;
  const humanInfo = _status.human || {};

  const humanLabel = humanInfo.is_typing ? '输入中' :
                     humanInfo.is_active ? '活跃' :
                     _status.human_zone || 'idle';
  const winTitle = _status.human_window || humanInfo.active_window || '';
  ...
  _panel.querySelector('.cw-state-row').innerHTML = `
    ...
    <div class="cw-zone-info">
      <span title="用户区域">\u{1F9D1} ${humanLabel}${winTitle ? ' · ' + winTitle : ''}</span>
      <span title="AI 区域">\u{1F916} ${_status.ai_zone || st}</span>
    </div>
    <div class="cw-meta-row">
      <span class="cw-meta-chip ${canOperate ? 'ok' : ''}" title="桌面操作权限">${canOperate ? '\u2705 可操作' : '\u{1F6AB} 等待'}</span>
      <span class="cw-meta-chip" title="运行中/排队">\u{1F3C3} ${running} 运行 · ${pending} 排队</span>
      ${_status.journal_count != null ? `<span class="cw-meta-chip" title="操作记录数">\u{1F4DD} ${_status.journal_count}</span>` : ''}
    </div>
  `;
  ...
  if (q.length === 0) {
    ...
    qEl.innerHTML = q.map((task, i) => `
      <div class="cw-queue-item">
        ...
        <span class="cw-qi-status cw-qs-${task.status || 'pending'}">${task.status || 'pending'}</span>
      </div>
    `).join('');
  }
  ...
  if (_journal.length === 0) {
    ...
    const entries = Array.isArray(_journal) ? _journal : (_journal.entries || []);
    jEl.innerHTML = entries.filter(j => !j.undone).slice(0, 10).map(j => {
      const icon = ACTION_ICONS[j.action || j.action_type] || ACTION_ICONS.default;
      const ts = normalizeTs(j.ts || j.timestamp);
      const desc = j.desc || j.description || j.action || '';
      const hasThumbs = j.has_before || j.has_after || j.has_thumbnails;
      return `
        <div class="cw-journal-item" data-id="${j.id}">
          <span class="cw-ji-icon">${icon}</span>
          <div class="cw-ji-info">
            <span class="cw-ji-desc">${desc}</span>
            <span class="cw-ji-time">${fmtAgo(ts)}</span>
          </div>
          ${hasThumbs ? `<button class="cw-thumb-btn" data-id="${j.id}" title="查看截图">\u{1F4F7}</button>` : ''}
  ...
  if (mockTag) mockTag.style.display = _useMock ? '' : 'none';
}
...
async function fetchStatus() {
  try {
    ...
    if (_status.state === 'working') {
      _status.can_operate_desktop = true;
      _status.tasks_running = 1;
      _status.tasks_pending = 2;
...
async function fetchJournal() {
  try {
    ...
    _journal = Array.isArray(data) ? data : (data.entries || data.journal || []);
    _useMock = false;
  } catch {
    _useMock = true;
...
async function showThumbnails(actionId) {
  try {
    const resp = await fetch(COWORK_API() + `/journal/${actionId}/thumbnails`);
    if (!resp.ok) return;
    const data = await resp.json();
    const overlay = document.createElement('div');
    overlay.className = 'cw-thumb-overlay';
    overlay.innerHTML = `
      <div class="cw-thumb-modal">
        <div class="cw-thumb-header">
          <span>操作前后对比</span>
          <button class="cw-close-btn" onclick="this.closest('.cw-thumb-overlay').remove()">\u2715</button>
        </div>
        <div class="cw-thumb-images">
          <div class="cw-thumb-col">
            <div class="cw-thumb-label">操作前</div>
            ${data.before ? `<img src="${data.before}" alt="before">` : '<div class="cw-empty">无截图</div>'}
          </div>
          <div class="cw-thumb-col">
            <div class="cw-thumb-label">操作后</div>
            ${data.after ? `<img src="${data.after}" alt="after">` : '<div class="cw-empty">无截图</div>'}
          </div>
        </div>
      </div>
    `;
...
function buildPanel() {
  ...
  _panel.addEventListener('click', (e) => {
    ...
    if (undoBtn) { doUndo(undoBtn.dataset.id); return; }
    const thumbBtn = e.target.closest('.cw-thumb-btn');
    if (thumbBtn) { showThumbnails(thumbBtn.dataset.id); return; }
  });
