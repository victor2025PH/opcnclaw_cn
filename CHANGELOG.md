...
## v5.3.0 (2026-03-22) — "Agent 真干活" 里程碑

### 核心突破：从"AI说话"到"AI干活"
- Agent 产出保存为真实文件（不再只是聊天文字）
- 项目工作空间：每个任务一个文件夹，打包 ZIP 下载就能用
- 程序员 Agent 自动提取代码块保存为 .py/.js/.html 文件
- 运营 Agent 任务含"发朋友圈"时自动调用 publish_moment
...
- 自由拖拽 + 点击飞行
- 双击输入 + 右键菜单 + 团队调度
- 每日早报（启动后自动播报天气/未读/系统）

### Agent 交互重构
- 先问需求再组建（不直接执行）
- 52 个 Agent 逐一报到（"老板好！我是XX，专长XX"）
- 用户确认后才开始（deploy_team → confirm_team）
- 完成后自动通知到聊天区（5 秒轮询）
- Agent 面板：右侧实时显示各人状态，点击可单聊
- 智能分配 AI 平台 + 无 Key 提醒 + 质量优先提示

### 测试与稳定
- 407 passed, 6 skipped
- 24 个工具（含 deploy_team/confirm_team/check_team_result）
- 52 角色 + 117 技能 + 15 模板
