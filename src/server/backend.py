...
class AIBackend:
    ...
    async def _chat_openai_stream(self, user_message: str, image_b64: str = None) -> AsyncGenerator[str, None]:
        ...
        # ── 2b. IoT 意图预处理（仅在 HomeAssistant 已配置时启用）─────
        _iot_configured = bool(os.environ.get("HA_URL") and os.environ.get("HA_TOKEN"))
        if not image_b64 and not skill_context and _iot_configured:
            try:
